...
// ─── useDownloadFile ──────────────────────────────────────────────────────────
// Two strategies depending on the URL type:
//
//  isDirectUrl = false  →  SECURE job, file lives on our backend
//                          Fetch blob directly (same-origin after API_BASE prefix)
//
//  isDirectUrl = true   →  FASTER job, file is a Google CDN URL
//                          CORS blocks a direct blob-fetch from the browser,
//                          so we route it through our backend proxy instead.
//
function useDownloadFile() {
  ...
  const downloadFile = async (
    ...
    if (!isDirectUrl) {
      // SECURE: prefix relative path with backend base URL
      // Check for double /api issue if API_BASE includes it
      let fullUrl = "";
      ...
      // If it turns out to be a full URL (e.g. from a bad state), treat as absolute
      if (downloadUrl.startsWith("http")) fullUrl = downloadUrl;

      await downloadBlob(fullUrl, filename);
    } else {
      // FASTER: Google CDN URL → pipe through our proxy to avoid CORS
      ...
      await downloadBlob(proxyUrl, filename);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 text-white text-[0.88rem] font-semibold hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-60"
      >
        {saving
