...
export class ItemsController {
    ...
    static search = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
        if (!req.user) return next(new AppError('Unauthorized', 401))
        ...
        const items = await ItemsService.searchItems(req.user.userId, q, limit ? parseInt(limit) : 10)
        res.status(200).json(new ApiResponse(200, items, 'Search completed successfully'))
    })
