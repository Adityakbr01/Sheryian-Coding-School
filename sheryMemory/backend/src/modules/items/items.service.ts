...
export class ItemsService {
  ...
  static async searchItems(
    ...
    if (query.length < 2) {
      logger.info(
        `[Items] 🚫 Query "${query}" too short. Skipping vector search.`,
      )
      return []
    }

    logger.info(
      `[Items] 🔍 Executing Hybrid Search for user ${userId} with normalized query: "${query}"`,
    )
    ...
    items.forEach((item, index) => {
      const semScore =
        typeof item.semantic_score === 'number'
          ? item.semantic_score
          : parseFloat(item.semantic_score)
      const kwScore =
        typeof item.keyword_score === 'number'
          ? item.keyword_score
          : parseFloat(item.keyword_score)
      const finalScore =
