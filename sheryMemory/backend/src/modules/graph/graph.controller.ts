...
export const syncGraph = catchAsync(async (req: AuthRequest, res: Response) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, null, 'Semantic graph relations synchronized'))
})

export const getRelatedItems = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const { itemId } = req.params
    const limit = parseInt(req.query.limit as string) || 5
    const data = await GraphService.getRelatedItems(
      req.user!.userId,
      itemId,
      limit,
    )
