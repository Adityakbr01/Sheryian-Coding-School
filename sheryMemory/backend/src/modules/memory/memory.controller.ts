...
export const getDueItems = catchAsync(async (req: AuthRequest, res: Response) => {
    const items = await MemoryService.getDueItems(req.user!.userId)
    res.status(200).json(new ApiResponse(200, items, `${items.length} items due for review`))
})

export const markReviewed = catchAsync(async (req: AuthRequest, res: Response) => {
