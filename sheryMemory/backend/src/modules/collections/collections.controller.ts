...
export const addItemToCollection = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const data = addItemToCollectionSchema.parse(req.body)
    const item = await CollectionsService.addItem(
      req.user!.userId,
      req.params.id,
      data.itemId,
    )
...
export const removeItemFromCollection = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, null, 'Item removed from collection'))
  },
)
