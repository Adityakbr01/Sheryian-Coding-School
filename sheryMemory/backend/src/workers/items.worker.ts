        ...
        try {
            ...
            if (scrapedData.content) {
                ...
                try {
                    ...
                    logger.info(`[Worker] ✅ Extracted deep semantic context successfully`)
                } catch (metaErr) {
            ...
            try {
                ...
                logger.info(`[Worker] 📡 Broadcasted early 'item_processed' socket event (Unblocked UI)`)
            } catch (socketErr) {
            ...
            if (scrapedData.content) {
                ...
                try {
                    ...
                    logger.info(`[Worker] 🧩 Generated Rich Text Embedding Input (${richTextForEmbedding.length} chars)`)
                    const vector = await EmbedderService.generateEmbedding(richTextForEmbedding)
                    if (vector) {
            ...
            try {
                const itemOwner = await prisma.item.findUnique({ where: { id: itemId }, select: { userId: true } })
                if (itemOwner) {
                    const io = getIo()
                    io.to(itemOwner.userId).emit('item_processed', { itemId })
                    logger.info(`[Worker] 📡 Broadcasted 'item_processed' socket event to user room: ${itemOwner.userId}`)
                }
            } catch (socketErr) {
