...
export async function generateTags(text: string): Promise<string[]> {
    try {
        const chain = tagPromptTemplate.pipe(LangchainClient.getChatInstance() as any)
        const response = await chain.invoke({ text: text.substring(0, 5000) }) as any
        ...
        const tags = JSON.parse(tagsString.replace(/```json/g, '').replace(/```/g, ''))
