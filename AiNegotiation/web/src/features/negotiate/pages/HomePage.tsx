...
export default function HomePage() {
  ...
  const handleStart = async () => {
    ...
    try {
      setLoading(true)
      const res = await negotiateApi.startSession(selectedProduct.id, difficulty)
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10 py-8">
      ...
      {/* Product Selection */}
      <div>
        <h2 className="mb-4 text-lg font-semibold text-(--text-primary)">Choose a Product</h2>
        {!isAuthenticated && (
          <div className="mb-4 rounded-xl border border-yellow-400 bg-yellow-50 p-4 text-sm text-yellow-800">
            🔒 Login is required to create a negotiation session. Please log in and then choose a product.
          </div>
        )}
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {products.map((p) => (
              ...
              >
                ...
                <div className="mt-2 font-bold text-(--accent)">₹{p.basePrice.toLocaleString()}</div>
              </button>
            ))}
          </div>
        )}
      ...
      {/* Difficulty */}
      <div>
        <h2 className="mb-4 text-lg font-semibold text-(--text-primary)">Select Difficulty</h2>
