...
export default function AdminPage() {
    ...
    const [productDrafts, setProductDrafts] = useState<Record<string, AdminProduct>>({})
    ...
    if (isAuthLoading) {
        return <div className="py-10 text-center text-(--text-muted)">Checking permissions...</div>
    }
    ...
    if (user?.role !== 'admin') {
        return (
            <div className="mx-auto max-w-2xl rounded-xl border border-(--border-default) bg-(--card-bg) p-8 text-center">
                <h1 className="text-2xl font-bold text-(--text-primary)">Admin Access Required</h1>
    ...
    return (
        <div className="mx-auto max-w-6xl space-y-6 py-6">
            ...
            <section className="rounded-xl border border-(--border-default) bg-(--card-bg) p-5">
                <h2 className="mb-4 text-xl font-semibold text-(--text-primary)">Create Product</h2>
                <form onSubmit={handleCreateProduct} className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <input
                        ...
                        onChange={(e) => setCreateForm((prev) => ({ ...prev, minimumPrice: e.target.value }))}
                        type="number"
                        min={1}
                    ...
                    <textarea
                        ...
                        className="md:col-span-2 min-h-24 rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2"
                    />
                    <button
                        type="submit"
                        disabled={createProductMutation.isPending}
                        className="md:col-span-2 rounded-md bg-(--accent) px-4 py-2 font-semibold text-(--text-on-accent) disabled:opacity-50"
                    >
            ...
            <section className="rounded-xl border border-(--border-default) bg-(--card-bg) p-5">
                <h2 className="mb-4 text-xl font-semibold text-(--text-primary)">Manage Products</h2>

                {productsQuery.isLoading ? (
                    <div className="py-6 text-sm text-(--text-muted)">Loading products...</div>
                ) : (
                    <div className="space-y-4">
                        {products.map((product) => {
                            const draft = productDrafts[product.id] ?? product
                            return (
                                <div key={product.id} className="rounded-lg border border-(--border-subtle) p-4">
                                    <div className="mb-2 flex items-center justify-between">
                                        <h3 className="text-sm font-bold text-(--text-primary)">{product.id}</h3>
                                        <span className={`text-xs font-semibold ${draft.isActive ? 'text-green-600' : 'text-red-600'}`}>
                                    ...
                                    <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
                                        <input
                                            value={draft.name}
                                            onChange={(e) => handleDraftChange(product.id, 'name', e.target.value)}
                                            className="rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2"
                                        />
                                        <input
                                            value={draft.emoji}
                                            onChange={(e) => handleDraftChange(product.id, 'emoji', e.target.value)}
                                            className="rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2"
                                        />
                                        <label className="flex items-center gap-2 rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2 text-sm">
                                            <input
                                                ...
                                                onChange={(e) => handleDraftChange(product.id, 'isActive', e.target.checked)}
                                            />
                                            Active
                                        </label>
                                        <input
                                            ...
                                            onChange={(e) => handleDraftChange(product.id, 'basePrice', Number(e.target.value))}
                                            className="rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2"
                                        />
                                        <input
                                            type="number"
                                            ...
                                            onChange={(e) => handleDraftChange(product.id, 'minimumPrice', Number(e.target.value))}
                                            className="rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2"
                                        />
                                        <textarea
                                            value={draft.description}
                                            onChange={(e) => handleDraftChange(product.id, 'description', e.target.value)}
                                            className="md:col-span-3 min-h-20 rounded-md border border-(--input-border) bg-(--input-bg) px-3 py-2"
                                        />
                                    </div>
                                    ...
                                    <div className="mt-3 flex gap-2">
                                        ...
                                        <button
                                            ...
                                            disabled={deactivateProductMutation.isPending || !draft.isActive}
            ...
            <section className="rounded-xl border border-(--border-default) bg-(--card-bg) p-5">
                <h2 className="mb-4 text-xl font-semibold text-(--text-primary)">Manage Users</h2>

                {usersQuery.isLoading ? (
                    <div className="py-6 text-sm text-(--text-muted)">Loading users...</div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody>
                                {users.map((entry) => {
                                    const targetUserId = entry._id ?? entry.id
                                    const roleDraft = targetUserId ? userRoleDrafts[targetUserId] ?? entry.role : entry.role
                                    const isCurrentUser = targetUserId === user?.id
                                    const unchanged = roleDraft === entry.role

                                    return (
                                        <tr key={targetUserId ?? entry.email} className="border-b border-(--border-subtle)">
                                            <td className="py-2 pr-2 font-medium">{entry.name}</td>
                                            <td className="py-2 pr-2">{entry.email}</td>
                                            <td className="py-2 pr-2">
                                                score {entry.score} | wins {entry.wins}/{entry.totalSessions}
                                            </td>
                                            <td className="py-2 pr-2">
                                                <select
                                                    value={roleDraft}
                                                    onChange={(e) => targetUserId && handleRoleChange(targetUserId, e.target.value as UserRole)}
                                            ...
                                            <td className="py-2">
                                                <button
                                                    ...
                                                    disabled={!targetUserId || isCurrentUser || unchanged || updateRoleMutation.isPending}
